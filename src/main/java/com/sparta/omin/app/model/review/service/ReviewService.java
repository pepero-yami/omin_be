...
public class ReviewService {
    ...
    public ReviewResponse createReview(User user, ReviewCreateRequest request, List<MultipartFile> images) {
        Order order = validateCreateRequestAndGetOrder(user, request, images);
        // Review 생성
        ...
        if (images != null && !images.isEmpty()) {
            ...
            newReview.addImages(uploadedUrls);
        }
        // DB 반영
        reviewRepository.save(newReview);
        reflectRatingStat(request, order);
    ...
    public Page<ReviewResponse> getReviews(ReviewCriteria criteria, Pageable pageable, UUID storeId) {
        return getReviewsResponseByRequest(criteria, pageable, storeId);
    }

    @Transactional
    public ReviewResponse updateReview(UUID reviewId, User user, ReviewUpdateRequest request, List<MultipartFile> images) {
        Review old = validateUpdateRequestAndGetReview(reviewId, user);
        if (request == null) {
            if (images.size() > 5) throw new OminBusinessException(ErrorCode.REVIEW_IMAGE_COUNT_EXCEEDED);
            handleImageAdd(old, images); // request 없이 이미지만 추가
        } else {
            validateUpdateRequestAndReflectStat(request, images, old);
    ...
    private void handleImageAdd(Review old, List<MultipartFile> newFiles) {
        boolean isDirty = false; // 연관 엔티티 수정 시 updatedAt 갱신여부 flag
        // 새 이미지 추가
        if (newFiles != null && !newFiles.isEmpty()) {
            if (old.getImages().size() + newFiles.size() > 5) {
                throw new OminBusinessException(ErrorCode.REVIEW_IMAGE_COUNT_EXCEEDED);
            }
            isDirty = isDirty(old, newFiles, isDirty);
            if (isDirty) {
                old.markUpdated();
            }
        }
    ...
    /**
     * 요청에 파일이 있으면 영속성 컨텍스트 플래그 반환
     */
    private boolean isDirty(Review old, List<MultipartFile> newFiles, boolean isDirty) {
    ...
    private void handleReviewImageUpdates(Review review, List<ReviewImage> updateImages, // 클라이언트가 원하는 순서대로 바꾼 ReviewImage
        ...
        if (newFiles != null && !newFiles.isEmpty()) {
            ...
            isDirty = isDirty(review, newFiles, isDirty);
        }
    ...
    private void reflectRatingStat(ReviewCreateRequest request, Order order) {
    ...
    private @NonNull Order validateCreateRequestAndGetOrder(User user, ReviewCreateRequest request, List<MultipartFile> images) {
        ...
        // 사용자가 요청한 주문 조회
        Order order = orderReadService.getOrder(loginUserId);
    ...
    private @NonNull Review validateUpdateRequestAndGetReview(UUID reviewId, User user) {
        // 리뷰 조회
    ...
    private @NonNull Page<ReviewResponse> getReviewsResponseByRequest(ReviewCriteria criteria, Pageable pageable, UUID storeId) {
        if (!criteria.equals(ReviewCriteria.DEFAULT)) {
            ...
            // 입력된 Criteria를 우선으로 Pageable에 쓰일 Sort생성
    ...
    private void validateUpdateRequestAndReflectStat(ReviewUpdateRequest request, List<MultipartFile> images, Review old) {
