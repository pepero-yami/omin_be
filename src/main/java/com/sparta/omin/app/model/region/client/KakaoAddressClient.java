...
public class KakaoAddressClient {
    ...
    public String normalizeToRegionDepth3(String query) {
        ...
        try {
            ...
            throw new IllegalStateException("카카오 주소 검색 API 호출에 실패했습니다.", e);
        }

        if (!response.getStatusCode().is2xxSuccessful() || response.getBody() == null) {
            throw new IllegalStateException("카카오 주소 검색 API 응답이 올바르지 않습니다.");
        }
        ...
        if (first == null || first.getAddress() == null) {
            throw new IllegalStateException("카카오 주소 검색 결과가 없습니다.");
        }
        ...
        if (!StringUtils.hasText(depth1) || !StringUtils.hasText(depth3)) {
            throw new IllegalStateException("카카오 주소 검색 결과에서 region depth 정보를 추출할 수 없습니다.");
        }
