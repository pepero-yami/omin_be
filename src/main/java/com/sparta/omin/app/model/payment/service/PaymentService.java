...
public class PaymentService {
        ...
        paymentRepository.findByOrderIdAndIsDeletedFalse(orderId)
                .ifPresent(p -> {
                    if(p.getPaymentStatus() == PaymentStatus.SUCCESS) {
                        throw new OminBusinessException(ErrorCode.INTERNAL_SERVER_ERROR, "이미 결제가 완료된 주문입니다.");
                    }
                });
    ...
    public PaymentResponse confirmPayment(UUID orderId, String paymentKey, double amount) {
        ...
        if (payment.getPaymentStatus() != PaymentStatus.READY) {
            throw new OminBusinessException(ErrorCode.INTERNAL_SERVER_ERROR, "결제 대기 상태가 아닙니다."); //FIXME 에러코드 추가하기
        }

        // 실제 연동은 안 하지만 금액 검증은 일단 넣음
        if (payment.getTotalPrice() != amount) {
            payment.fail();
            throw new OminBusinessException(ErrorCode.INTERNAL_SERVER_ERROR, "결제 금액이 일치하지 않습니다.");
        }
