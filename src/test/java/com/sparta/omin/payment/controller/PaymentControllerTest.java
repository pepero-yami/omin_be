...
class PaymentControllerTest extends PaymentControllerHelper {
    ...
    void getPaymentsByAdmin_success() throws Exception {
        ...
        PaymentResponse response = new PaymentResponse(
                UUID.randomUUID(), UUID.randomUUID(), PaymentMethod.CREDIT_CARD, 25000.0, LocalDateTime.now(), status, "key");

        given(paymentService.getPayments(eq(customerId), eq(status), any())).willReturn(new PageImpl<>(List.of(response), PageRequest.of(0, 10), 1));
        ...
        mockMvc.perform(get("/api/v1/admin/payments")
                ...
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content[0].paymentStatus").value("SUCCESS"));
    }
